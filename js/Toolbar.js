...
Toolbar.prototype.init = function()
{
	...
	this.addButton('undo','Insert Table', function()
	{
		var dlg = new BidtracerProductDialog(ui);
		ui.showDialog(dlg.container, 500, 700, true, false, null, false);


		//dlg.init();

		//ui.bom.Insert();
		// var v1 = graph.insertVertex(graph.getDefaultParent(), 'BOM', '<table border="1" width="100%" height="100%" cellpadding="4" style="width:100%;height:100%;border-collapse:collapse;">' +
		// '<caption style="font-size:16px;">BILL OF MATERIALS</caption>'+
		//  '<tr style="background-color:#858585;color:#ffffff;border:1px solid #98bf21;"><th align="left">DID</th><th align="left">DESCRIPTION</th><th align="left">MANUFACTURER</th><th align="left">PART NUMBER</th><th align="left">QTY</th></tr>' +
		//  '<tr style="border:1px solid #98bf21;"><td></td><td></td><td></td><td></td><td></td></tr>' +
		// '</table>', 40, 40, 80, 30, 'text;html=1;strokeColor=#c0c0c0;fillColor=#ffffff;overflow=fill;rounded=0;');
		...
		// var v1 = selectedCells[0].value;
		// console.log(selectedCells[0].value.nodeName);
		...
		// selectedCells[0].setValue(person1);

	});

	this.addButton('redo','Insert Row', function()
	{
		var groupedCell=null;

		if (graph.getSelectionCount() == 1)
		{
			groupedCell=graph.getSelectionCell();
			graph.setCellStyles('container', '1');
		}
		else
		{
			groupedCell = graph.groupCells(null, 0);
			graph.setSelectionCell(groupedCell);
		}

		var value = graph.getModel().getValue(groupedCell);
		...
		value.setAttribute("NodeId", "12345");
		// Updates the value of the cell (undoable)
		graph.getModel().setValue(groupedCell, value);


		//graph.toggleCellStyles(mxConstants.STYLE_EDITABLE,1);
		// var bom = graph.model.getCell('BOM');
		// var valueXml = bom.getValue();
		// var xmlDoc = mxUtils.parseXml(valueXml);

		// var parentTable = xmlDoc.getElementsByTagName("tr");
		// for(var i=0;i<parentTable.length;i++)
		// {
		// 	console.log(parentTable[i].firstChild);
		// }

		// var row = xmlDoc.createElement("tr");

		// row.setAttribute("style","border:1px solid #98bf21;");
		
		// var col1 = xmlDoc.createElement("td");
		// col1.textContent = "Value 13";
		// row.appendChild(col1);

		// var col2 = xmlDoc.createElement("td");
		// col2.textContent = "Value 14";
		// row.appendChild(col2);

		// var col3 = xmlDoc.createElement("td");
		// col3.textContent = "Value 15";
		...
		// bom.setValue(mxUtils.getXml(xmlDoc));
		// graph.updateCellSize(bom);
		// }
		// finally
		// {
...
Toolbar.prototype.createButton = function(classname)
{
	...
	// iconElt.className='geSprite ' + classname;
