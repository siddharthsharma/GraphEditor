...
this.graph.addListener(mxEvent.EDITING_STARTED, function(sender, evt)
{
	text = graphs.cellEditor.textarea;

	mxEvents.addListener(text, 'keydown', function (evt)
{
//   if (!(evt.keyCode >= 48 && evt.keyCode <= 57) &&
//       !(evt.keyCode >= 96 && evt.keyCode <= 105))
//   {
//     mxEvents.consume(evt);
//   }
...
(function()
{
	...
	{
		if (this.div != null && this.factoryMethod != null)
		{
			...
			var parentListItem = evt.currentTarget.parentElement;

			// if(parentListItem.children.length>1)
			// {
			// 	var dropDownMenu = parentListItem.querySelector(".dropdown-menu");
				
			// 	if(dropDownMenu!=null)
			// 	{
					
			// 		parentListItem.removeChild(dropDownMenu);
			// 	}
			// }
			...
			// {
				...
				{
					mxEvent.release(this.div.firstChild);
				...
				if (this.itemCount > 0)	
				{
					...
					//parentListItem.appendChild(this.unOrderedList);
			...
			// 		//this.div.classList.toggle("show");
			// 		parentListItem.appendChild(this.div);
					
			// 		//mxUtils.fit(this.unOrderedList);
			// 		// this.showMenu();
			// 		//this.fireEvent(new mxEventObject(mxEvent.SHOW));
			// 	}
			// }
	...
	mxPopupMenu.prototype.init = function()
	{
		...
		//this.div.className='dropdown-menu multi-level';
		...
		//this.div.appendChild(this.unOrderedList);
	...
	mxPopupMenu.prototype.addSeparator = function(parent, force)
	{
		...
		else
		{
			...
			if(parent.constructor===mxPopupMenu || parent.constructor===mxPopupMenuHandler)
			{
				this.div.appendChild(separator);
			}
			else
			{
				this.createSubmenu(parent).appendChild(separator);
			}
		}
	}
...
// }
	...
	mxPopupMenu.prototype.addItem = function(title, image, funct, parent, iconCls, enabled, active)
	{
		...
		if (this.labels)
		{
			...
			{
				elt.setAttribute('role','menuitem');
			}

			elt.classList.add('dropdown-item');
			
			elt.setAttribute('href','javascript:void(0)');
			
			elt.setAttribute('tabindex','-1');

			mxEvent.addGestureListeners(elt, funct);
			...
			eltli.appendChild(elt);
		}
		
		if(parent.constructor===mxPopupMenu || parent.constructor===mxPopupMenuHandler)
		{
			this.div.appendChild(eltli);
		}
		else
		{
			this.createSubmenu(parent).appendChild(eltli);
		}
	...
	{
		var anchorMenuItem = item.firstChild;
		var imageTag = document.createElement("img");
		imageTag.src = img;
		anchorMenuItem.appendChild(imageTag);
	}

	mxPopupMenu.prototype.createSubmenu = function(parent)
	{
		var existingSubMenu = parent.querySelector(".dropdown-menu");
		
		if(existingSubMenu==null)
		{
			parent.className+='dropdown-submenu';
			var subMenuList = document.createElement('ul');
			subMenuList.className='dropdown-menu multi-level';
			parent.appendChild(subMenuList);
			return subMenuList;
		}
		else
		{
