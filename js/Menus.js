...
Menus.prototype.init = function()
{
	...
	this.put('formatBlock', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		...
		addItem('', 'h1').firstChild.innerHTML = '<h1 style="margin:0px;">' + mxResources.get('heading') + ' 1</h1>';
		addItem('', 'h2').firstChild.innerHTML = '<h2 style="margin:0px;">' + mxResources.get('heading') + ' 2</h2>';
		addItem('', 'h3').firstChild.innerHTML = '<h3 style="margin:0px;">' + mxResources.get('heading') + ' 3</h3>';
		addItem('', 'h4').firstChild.innerHTML = '<h4 style="margin:0px;">' + mxResources.get('heading') + ' 4</h4>';
		addItem('', 'h5').firstChild.innerHTML = '<h5 style="margin:0px;">' + mxResources.get('heading') + ' 5</h5>';
		addItem('', 'h6').firstChild.innerHTML = '<h6 style="margin:0px;">' + mxResources.get('heading') + ' 6</h6>';
		
		addItem('', 'pre').firstChild.innerHTML = '<pre style="margin:0px;">' + mxResources.get('formatted') + '</pre>';
		addItem('', 'blockquote').firstChild.innerHTML = '<blockquote style="margin-top:0px;margin-bottom:0px;">' + mxResources.get('blockquote') + '</blockquote>';
	})));
...
Menus.prototype.createPopupMenu = function(menu, cell, evt)
{
	...
	{
		menu.addItem('Refresh BOM', null, function() { ui.bom.Refresh(); }, null);	
	}
	else
	{
		menu.addItem('Add BOM', null, function() { ui.bom.Insert(); }, null);
	}
...
Menubar.prototype.addMenu = function(label, funct, before)
{
	...
	eltli.className='nav-item dropdown';

	var elt = document.createElement('a');
	//elt.className = 'geItem';
	elt.className = 'nav-link dropdown-toggle';
	elt.setAttribute('href','javascript:void(0)');
	elt.setAttribute('role','button');
	elt.setAttribute('data-toggle','dropdown');
    ...
    if (before != null)
    {
    	this.container.insertBefore(eltli, before);
    }
    else
    {
    	this.container.appendChild(eltli);
    }
